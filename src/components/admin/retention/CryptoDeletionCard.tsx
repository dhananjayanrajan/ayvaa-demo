...
export function CryptoDeletionCard() {
  return (
    <motion.div variants={rise}>
      <div className="relative overflow-hidden rounded-[26px] bg-gradient-to-br from-slate-950 via-slate-900 to-teal-950 p-5 shadow-[0_28px_64px_-30px_rgba(2,12,24,0.85)]">
        <div aria-hidden className="pointer-events-none absolute -right-14 -top-16 h-48 w-48 rounded-full bg-sky-500/20 blur-3xl" />
        <div aria-hidden className="pointer-events-none absolute -bottom-16 -left-12 h-40 w-40 rounded-full bg-indigo-400/10 blur-3xl" />
            ...
            {cryptoRules.map((rule, i) => (
              <div key={rule.text}>
                ...
                <div className="flex items-center gap-3 px-3.5 py-3">
                  ...
                  <span className="min-w-0 flex-1 break-words text-[12.5px] font-semibold leading-snug text-sky-50/80">{rule.text}</span>
                </div>
              </div>
            ))}
          </div>
