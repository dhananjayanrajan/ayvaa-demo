...
export function SheetShell({ icon, tone, title, subtitle, onClose, children, footer }: Props) {
  return (
    <motion.div
      initial={{ y: '100%' }}
      animate={{ y: 0 }}
    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 pb-4 pt-3.5">{children}</div>

      <div className="shrink-0 px-5 pb-6 pt-3.5">{footer}</div>
    </motion.div>
  )
}
